...
(function($) {
	...
	var generateFontCss = function( fonts ) {
		...
		if ( 'object' === typeof fonts ) {
			...
			for ( var key in fonts ) {
				...
				switch ( ext ) {
					...
					case 'woff2' :
						...
						break;
					case 'woff' :
						line4 = "        url('"+url+"') format('woff'),\n";
						break;
					case 'ttf' :
						line5 = "        url('"+url+"') format('truetype'),\n";
						break;
					case 'svg' :
						line6 = "        url('"+url+"#"+name+"') format('svg');\n";
						break;
					case 'otf' :
						line7 = "    src: url('"+url+"');\n";
						break;
			...
			css += line1 + line2 + line3 + line4 + line5 + line6 + line7;
	...
	$body.on('click', '.wpcanvas2-image-upload', function( event ) {
		...
		if ( 'wpcanvas2_insert_single' === options.state ) {
			// add the single insert state
			file_frame[frame_key].states.add([
				// Main states.
				new media.controller.Library({
					id:         'wpcanvas2_insert_single',
		...
		else if ( 'wpcanvas2_insert_multi' === options.state ) {
			// add the single insert state
			file_frame[frame_key].states.add([
				new media.controller.Library({
					id:         'wpcanvas2_insert_multi',
					title: clicked.data( 'title' ),
					priority:   20,
					toolbar:    'select',
					filterable: 'uploaded',
		...
		else if ( 'wpcanvas2_insert_fonts' === options.state ) {
			// add the single insert state
			file_frame[frame_key].states.add([
				new media.controller.Library({
					id:         'wpcanvas2_insert_fonts',
					title: clicked.data( 'title' ),
					priority:   20,
					toolbar:    'select',
					filterable: 'uploaded',
		...
		file_frame[frame_key].on( 'select update insert', function(e) {
			...
			if ( target.length ) {
				if ( 'wpcanvas2_insert_fonts' === options.state ) {
					var at_font_face = target.val() + "\n" + generateFontCss( values );
					target.val( at_font_face.trim() );
				}
				else {
	...
	.on('click', '.wpcanvas2-restore-image', function( e ) {
		e.preventDefault();
	...
	.on('click', '.wpcanvas2-delete-image', function( e ) {
		e.preventDefault();
